...
declare namespace MenuSDK {
	/**
	 * Lines and rings over the world. RmlUi has no path API, so a run is a quad rotated about its left
	 * edge with a shader carving the rounded cap inside it, and a ring is the sdf circle's border.
	 * Everything a frame draws goes onto one pooled surface, the same one the HUD cards use.
	 *
	 * @example
	 * canvas.Begin()
	...
	class WorldCanvas {
		...
		/** Connects the points that project on screen, skipping the ones behind the camera. */
